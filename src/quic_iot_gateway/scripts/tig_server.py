...
import os
import configparser
from quic_iot_gateway.tls.transport import init_tls_server
from quic_iot_gateway.context import CoAPServerContext, MQTTSNGWServerContext
from quic_iot_gateway.tls.gateway import iot_gateway_server_protocol_factory
from quic_iot_gateway.utils import setup_logger

logger = logging.getLogger(__name__)


async def asyncio_main(tls_server_host, tls_server_port,
                       cert_file, key_file, disable_cert_verification,
                       mqtt_broker_host, mqtt_broker_port):
    await init_tls_server(
        tls_server_host,
        tls_server_port,
        cert_file,
        key_file,
...
def main():
    ...
    tls_server_host = args.tls_server_host or config.get('tls_server', 'host', fallback='0.0.0.0')
    tls_server_port = args.tls_server_port or config.getint('tls_server', 'port', fallback=1234)
    cert_file = args.cert_file or config.get('tls_server', 'cert_file', fallback='cert.pem')
    key_file = args.key_file or config.get('tls_server', 'key_file', fallback='key.pem')
    ...
    asyncio.run(asyncio_main(tls_server_host, tls_server_port,
                             cert_file, key_file, disable_cert_verification,
                             mqtt_broker_host, mqtt_broker_port))
